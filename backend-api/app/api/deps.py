...
async def get_authenticated_user(
    ...
    if auth:
        ...
        user: TokenUser = parse_token(token.access_token)
        return user
    return None


def parse_token(token: str) -> TokenUser | None:
    try:
        payload = jwt.decode(token, settings.JWT_SECRET, algorithms=["HS256"])
        email: str = payload.get("sub")
        if email is None:
            raise credentials_exception
        user: TokenUser = TokenUser(**payload.get("user"))
        return user
